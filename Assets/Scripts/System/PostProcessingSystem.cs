...
public class PostProcessingSystem : MonoBehaviour
{
    ...
    //SET DEPTH OF FIELD
    public static void SetDepthField(float endValue, float time, bool loop)
    {
        int timeLoop = (loop) ? 2 : 0;
        DepthOfField depthOfField = postProcess.profile.TryGet<DepthOfField>(out depthOfField) ? depthOfField : null;
        //DOVirtual.Float(depthOfField.farFocusStart.value, endValue, time, x => depthOfField.farFocusStart.value = x).SetLoops(timeLoop, LoopType.Yoyo);
        //DOVirtual.Float(depthOfField.farFocusEnd.value, endValue, time, x => depthOfField.farFocusEnd.value = x).SetLoops(timeLoop, LoopType.Yoyo);
        DOVirtual.Float(depthOfField.farMaxBlur, endValue, time, x => depthOfField.farMaxBlur = x).SetLoops(timeLoop, LoopType.Yoyo);
    }
